...
export default {
    ...
    methods: {
        update_changed() {
            this.changed = ( (this.deviceProp.device != this.editDevice.device) || 
                             (this.deviceProp.name != this.editDevice.name) )
            console.log("Changed is now",this.changed)
        },
        update_type() {
            axios.put("/api/device/"+this.editDevice.id+"/",this.editDevice)
                .then((result) => {
                    console.log(result)
...
<div class="row rounded mt-1 mb-0 p-0 ">
    <input class="col bg-secondary rounded me-1" v-model="editDevice.name" v-on:change="update_changed" />
    <input class="col bg-secondary rounded" v-model="editDevice.device" v-on:change="update_changed"/>
